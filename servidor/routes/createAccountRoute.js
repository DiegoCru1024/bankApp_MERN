...
function generateUniqueID() {
    ...
    return accountModel
        .findOne({accountID: generatedID.toString()})
        .then((accountData) => {
            if (accountData) {
...
router.post('/', (req, res) => {
    ...
    accountModel
        .findOne({
            ownerUserID: req.body.ownerUserID.toString(),
            accountName: req.body.accountName.toString(),
        })
        .then((existingAccount) => {
            if (existingAccount) {
                ...
                return res
                    .status(400)
                    .send({message: 'El usuario ya tiene una cuenta registrada con ese nombre'});
            }

            // Generar un ID único
            return generateUniqueID().then((generatedID) => {
                // Asignar el ID generado al campo accountID
                req.body.accountID = generatedID;

                // Guardar la nueva cuenta
                return new accountModel(req.body)
                    .save()
                    .then(() => {
